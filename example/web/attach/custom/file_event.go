...
func NewFileEvent() attachment.FileEventer {
	dir := conf.GetData().AttachConfig.Dir
	logFile := conf.GetData().AttachConfig.LogFile
	...
	return &fileEvent{
		dir:       dir,
		file:      f,
		saveLocal: conf.GetData().AttachConfig.Enable,
		saveMinio: conf.GetData().AttachConfig.MinioConfig.Enable,
	}
}
	...
	case attachment.ProgressStageSuccessQuit:
		...
		for name, pack := range progress.Record {
			...
			if f.saveMinio {
				...
				path := fmt.Sprintf("/%s/%s", phone, name)
				if url, err := minioFile.Default().Upload(path, pack.StreamBody); err != nil {
					str += fmt.Sprintf("\n 上传到minio[%s] 保存情况[%v]\n", url, err)
				}
			}
		}
