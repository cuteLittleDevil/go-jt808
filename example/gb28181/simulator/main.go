...
func main() {
	...
	interval := time.Duration(conf.GetData().GB28181.CreateIntervalMicrosecond) * time.Microsecond
	for i := 1; i <= device.MaxID; i++ {
		wg.Add(1)
		go func(i int) {
			defer wg.Done()
			sim := "0000000" + fmt.Sprintf("%d", i)
			id := "3402000000132" + sim[len(sim)-7:]
			client := gb28181.New(sim, gb28181.WithPlatformInfo(gb28181.PlatformInfo{
				...
				gb28181.WithKeepAliveSecond(conf.GetData().GB28181.KeepAliveSecond), // 心跳保活周期30秒
			...
			go client.Run()
			if second := conf.GetData().GB28181.StopSecond; second > 0 {
